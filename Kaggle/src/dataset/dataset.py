...
height_per_cent   = int(len(data) * 0.8)
training_data     = np.array(data[:height_per_cent])
training_labels   = np.array(labels[:height_per_cent])
validation_data   = np.array(data[height_per_cent:])
validation_labels = np.array(labels[height_per_cent:]) 

# Lecture du fichier de test
final_test = h5py.File("../kaggle_lille1_2018_test_v1.save", 'r')
final_data = np.array(final_test["dataset_1"])

def variance_filter(limit):
  variances = np.var(training_data.T, axis=1) 
  where_variances = np.where(variances > limit)[0]
  return np.take(training_data, where_variances, axis=1), np.take(validation_data, where_variances, axis=1), np.take(final_data, where_variances, axis=1)

def covariance_filter():
